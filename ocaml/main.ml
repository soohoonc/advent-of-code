...
let command =
  let year_param = Command.Param.anon (Command.Param.( %: ) "year" Command.Param.int) in
  let day_param = Command.Param.anon (Command.Param.( %: ) "day" Command.Param.int) in
  Command.async ~summary:"Start Advent of Code Solution"
    (Command.Param.map2 year_param day_param
      ~f:(fun year day ->
        fun () ->
