...
let () =
  ...
  Command.async ~summary:"Start Advent of Code Solution"
    (Command.Param.return (fun () ->
         ...
         Aoc.Input.get_input ~session ~year:2024 ~day:2 >>| function
         | Ok content ->
             let part_one = Aoc.Day_two.solve_part_one content in
             let part_two = Aoc.Day_two.solve_part_two content in
             Core.Printf.printf "Part 1: %d\nPart 2: %d\n" part_one part_two
         | Error e ->
             print_endline @@ Printf.sprintf "Error Fetching Input: %s" e))
  |> Command_unix.run
